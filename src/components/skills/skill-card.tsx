...
export function SkillCard({ skill, isInstalled, isInstalling, onInstall, onClick }: SkillCardProps) {
  ...
  return (
    ...
    >
      ...
      <div onClick={(e) => e.stopPropagation()}>
        {isInstalled ? (
          <span className="font-mono text-[10px] px-3 py-1 bg-white/[0.05] border border-white/[0.08] text-white/40 rounded-[2px]">
            ✓ Installed
          </span>
        ) : isInstalling ? (
          ...
          <button
            className="font-mono text-[10px] px-3 py-1 bg-emerald-900/20 border border-emerald-800/30 text-emerald-400 hover:bg-emerald-900/40 rounded-[2px] transition-colors"
